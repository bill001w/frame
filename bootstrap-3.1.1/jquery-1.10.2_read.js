...
(function (window, undefined) {
    ...
    // 生成一个 options 配置对象，生成的配置对象就是{once:true, memory:true}
     ...
     *
     */
    // options参数包含四个可选项，可用空格或者, 分隔，分别是once、memory、unique、stopOnFalse
    jQuery.Callbacks = function (options) {
        // Convert options from String-formatted to Object-formatted if needed
        // (we check in cache first)
        // 通过字符串在optionsCache寻找有没有相应缓存，如果没有则创建一个，有则引用
        options = typeof options === "string" ?
            // 如果传递的是字符串，可以为如"once memory"
            // 生成的配置对象就是{once:true, memory:true}
            (optionsCache[options] || createOptions(options)) :
            // 如果传递的是对象，可以为如{once:true, memory:true}
        ...
        var
            ...
            // 如果参数设置为once，stack为false，否则为[]（1个参数两用了）
            // stack为参数不为once时，当回调列表正在fire时，又有另外的fire情况下，保存的fire参数。
            stack = !options.once && [],

            // Fire callbacks
            // 触发回调函数列表，内部使用的辅助函数，私有方法，被 self.fire 以及 self.fireWith 调用
            fire = function (data) {
                // 如果参数 memory 为true，则记录 data（下次用，所以是上次的参数）
                ...
                if (list) {
                    if (stack) {
                        ...
                        if (stack.length) {
                            // 从堆栈头部取出，递归fire
                            // 这里是深度（递归，因为stack保存的多次fire的参数，因此递归时前面的list多次fire也应该）遍历，直到stack为空
                            fire(stack.shift());
                        }
                    } else if (memory) {
                        // 如果是"once memory" ，光memory不行，会只走上面的stack，只有同时once，stack为false才行。
                        // 如果memory，且经过lock，此时stack也为undefined，也行
                        // 这样的作用是：因为add方法中没有使用fireWith，而是直接调用的fire方法，因此仍然可以继续fire，清空后不会触发之前的回调
                        list = [];
                    } else {
                        // once
                        self.disable();
            ...
            // 实际的 callbacks 对象，var callbacks = $.Callbacks() 最后返回的就是这个 sele 对象
            self = {
                // 向回调列表中添加一个回调或回调的集合。可以是一个函数，或者一个函数数组，最终会被压扁。
                ...
                add: function () {
                    ...
                    if (list) {
                        ...
                        // 遍历传过来的 arguments 进行 push
                        (function add(args) {
                            jQuery.each(args, function (_, arg) {
                                var type = jQuery.type(arg);
                                // 类型判断，如果传入的是单个方法
                                if (type === "function") {
                                    // 不要求unique或者当前队列还没有该回调
                        ...
                        if (firing) {
                            ...
                            // memory有值，说明已经 fire 过并且是 memory 类型的管理器，值是上一次 fire 的 [context, args]
                        } else if (memory) {
                            firingStart = start;
                ...
                // 禁用回调列表中的回调。禁用掉之后，把里边的队列、栈等全部清空了！无法再恢复
                ...
                // 所谓锁定，就是以后没法再fire，因为stack为undefined后，等同于once，详解fireWith源码判断部分
                ...
                fireWith: function (context, args) {
                    // list 不为空，并且参数不是once（once的话stack将为false）
                    // 如果锁定，stack为undefined，效果等同于once
                    if (list && (!fired || stack)) {
                        args = args || [];
                        // 把 args 组织成 [context, [arg1, arg2, arg3, ...]]，第一个参数是上下文
                        args = [context, args.slice ? args.slice() : args];

                        if (firing) {
                            // 如果当前还在 firing，将参数推入堆栈，等待当前回调结束再调用
                            stack.push(args);
                        } else {
                            // 否则直接调用
                            fire(args);
    ...
    jQuery.extend({
        ...
        queue: function (elem, type, data) {
            ...
            // elem 存在（必须存在，否则不返回）
            if (elem) {
                // 拼接队列名，为type+queue 或者 fxqueue，不传入队列名则默认为后者（动画队列）
                type = (type || "fx") + "queue";

                // jQuery._data() 添加或读取一个仅供内部使用的数据，这里是取出队列
        ...
        dequeue: function (elem, type) {
            ...
            var queue = jQuery.queue(elem, type),
                ...
                // 如果我们调用的是 $.dequeue(document,"q1") 的话，那么属性对象名就是 q1queueHooks，
        ...
        // 队列钩子，创建1个queueHooks缓存对象，或者是返回当前有的那个
        // 涉及到队列的缓存包括2类：队列缓存type + "queue"，以及队列钩子自身的这个缓存queueHooks即type + "queueHooks"
        // 队列钩子缓存对象内有empty属性，为1个回调队列，执行回调会清理对应缓存。在后面还有stop方法等
        // 因为只要有队列，队列没有结束，就不会执行empty，因此还可以用它来判断队列是否存在
        _queueHooks: function (elem, type) {
            // key 名为 type + "queueHooks"
            var key = type + "queueHooks";

            // jQuery._data(elem, key) 存在，则直接返回
            // 否则，添加一个 keyqueueHooks 对象（有empty属性，值为1个回调函数管理器）
    ...
    jQuery.fn.extend({
        ...
        // 例子：$(this).animate({width:300},2000).animate({left:300},2000);$(this).promise().done(function(){alert(3)});
        // 意思是，等上面两个定时器函数都执行结束后（因为他们默认处理的都是 fx 队列）。才会执行弹出 alert(3) 的函数
        promise: function (type, obj) {
            ...
            while (i--) {
                ...
                // queueHooks 缓存如果还存在，说明队列还没完
                // empty是1个回调列表，每个队列结束后会执行hooks.empty.fire，当count=0时，会触发resolve，从而触发添加的回调
                if (tmp && tmp.empty) {
                    count++;
    ...
    jQuery.extend({
        ...
        // val钩子
        valHooks: {
            // 如果没有对此 option 显式设置 value 值（即，没有value这个属性），获取到的值是 option 的 text ，也就是 option 的文本
            // 但是 IE6-7 下获取到的值是""
            option: {
                get: function (elem) {
    ...
    jQuery.fn.extend({
        ...
        // fn: 事件被触发时，执行的函数
        // 实质只完成一些参数调整的工作，而实际负责事件绑定的是其内部 jQuery.event.add 方法
    ...
    jQuery.ajaxPrefilter("json jsonp", function (s, originalSettings, jqXHR) {
        ...
        var callbackName, overwritten, responseContainer,
            //先在s.url中寻找jsonp标志'anyCallbackName=?',如果未找到那么尝试在s.data中找标志"anyCallbackName=?" 。
        ...
        if (jsonProp || s.dataTypes[0] === "jsonp") {
            ...
            //s.jsonpCallback参数不推荐是手动设置，而是通过jQuery随机生成。
            ...
            //s.url或s.data中插入了"anyCallbackName=?"标志
            if (jsonProp) {
                s[jsonProp] = s[jsonProp].replace(rjsonp, "$1" + callbackName);

                //其它情况，即s.url和s.data中都没有手动插入"fun=?"标志，那么自动生成。
            } else if (s.jsonp !== false) {
                s.url += (ajax_rquery.test(s.url) ? "&" : "?") + s.jsonp + "=" + callbackName;
            }

            // Use data converter to retrieve json after script execution
            //设置script类型到json类型的转换，因为当前函数最后会return "script";
            s.converters["script json"] = function () {
                if (!responseContainer) {
                    jQuery.error(callbackName + " was not called");
                }
                //转换器返回的是后面给window注册的那个回调函数的返回值，即响应返回值
            ...
            //在window上注册1个同名回调函数，将来会调用这个回调函数（jsonp）
            window[callbackName] = function () {
                //这里的arguments是这个回调函数自身的arguments
    ...
    var fxNow, timerId,
        ...
        rrun = /queueHooks$/,
        // 动画预处理回调数组，在动画执行前对动画属性、值等处理
        animationPrefilters = [defaultPrefilter],
        // 动画属性修正回调
        tweeners = {
            // 在动画开始前，将参数转为设置属性的开始值、结束值和一致的单位
            "*": [function (prop, value) {
                // this指向animation对象
                // 返回一个Tween构造函数实例
                var tween = this.createTween(prop, value),
                    // 获取属性当前样式值
                    target = tween.cur(),
                    // value就是设置值，rfxnum分别匹配+=、数字、单位，parts[3]为单位，unit为目标值的单位
                    // jQuery.cssNumber里的属性不需要单位
                    parts = rfxnum.exec(value),
                    unit = parts && parts[3] || (jQuery.cssNumber[prop] ? "" : "px"),

                    // 属性为非像素样式
                    // prop为jQuery.cssNumber中的样式，或unit部位px且当前属性值也不能转换为除0外的数字
                    // 使用rfxnum匹配当前样式到start数组中
                ...
                // 现在的值和设置的值单位不一样处理，将开始值转换为结束值单位一样的数字
                if (start && start[3] !== unit) {
                    ...
                    // 更新start为当前样式target，使用前面的变量start，节约1个变量
                    // Iteratively approximate from a nonzero starting point
                    start = +target || 1;

                    do {
                        // 针对scale为0或NaN情况，scale在while条件中更新为tween.cur() / target
                        ...
                        // 计算开始值对应单位的值
                        // 如果是像素，开始值 = 结束值 / 结束值的像素值 × 开始值的像素值
                        // Adjust and apply
                        start = start / scale;
                        jQuery.style(tween.elem, prop, start + unit);

                        // 循环，一直到scale不变或完成
                        // 更新scale = tween.cur() / target，判断和自身是否相等，针对tween.cur()或target可能为0或NaN情况
                        // 兼容处理是通过maxIterations不能超过20次
    ...
    // 遍历并执行tweeners中prop属性保存的回调，用于修正属性
    ...
    function Animation(elem, properties, options) {
        var result,
            ...
            tick = function () {
                ...
                // 生成进度报告，tick函数每调用一次，options.progress就执行一次
        ...
        // 将是动画属性转换成驼峰式，并设置动画属性
        propFilter(props, animation.opts.specialEasing);

        // 遍历动画预过滤器，并执行回调
        // 其中defaultPrefilter为默认预过滤器，每次都会执行，无返回值，可以通过jQuery.tweener()进行了拓展hook
        for (; index < length; index++) {
    ...
    function propFilter(props, specialEasing) {
        ...
        for (index in props) {
            ...
            // expand扩展，将margin/padding/border扩展为四个方向名值对形式
    ...
    // 将内部函数Animation添加为jQuery的1个静态方法
    jQuery.Animation = jQuery.extend(Animation, {
        // 扩展tweeners方法
        // props如"ss * sd"可以变成["ss","*","sd"]，遍历其中每个属性对应的堆栈推入callback在栈顶
        tweener: function (props, callback) {
            if (jQuery.isFunction(props)) {
                callback = props;
                props = ["*"];
            } else {
                //props可以把"ss * sd"变成["ss","*","sd"]
                props = props.split(" ");
            }
     ...
     *
     * defaultPrefilter有4个作用：
     * 1、对于非队列动画执行时，启动计数，当非队列动画结束，执行queuehook自毁函数
     * 2、对于涉及到height/width动画，overflow都要设置为"hidden"，动画完成后恢复。这个有利于提高渲染速度。
     * 3、对于涉及到height/width的inline元素动画，需要设置为inline-block，jq中的设置时display为none的也要变为显示运动。
     * 4、支持属性值为toggle/show/hide，会被修正为适当的值。
     *
     */
    function defaultPrefilter(elem, props, opts) {
        ...
        // 不将动画放入默认队列情况，通过hooks.unqueued计数，确保动画执行完后清理缓存
        // handle queue: false promises
        if (!opts.queue) {
            // 获取或者设置动画队列钩子，钩子有empty属性，值为1个回调，执行会清理对应缓存
            ...
            if (hooks.unqueued == null) {
                ...
                hooks.empty.fire = function () {
                    // 当hooks.unqueued为0时，执行清除动画队列缓存
                    if (!hooks.unqueued) {
                        oldfire();
                    }
                };
            }
            // 每执行1次defaultPrefilter加1
            hooks.unqueued++;
        ...
        // height/width动画时，对overflow修正 + inline元素修正
        ...
        if (elem.nodeType === 1 && ("height" in props || "width" in props)) {
            ...
            // 如果为inline且又不为float时，转为inline-block
        ...
        // 把overflow改为hidden（前提是上面的if条件成立，即width或height动画情况）
        if (opts.overflow) {
        ...
        // 遍历动画属性，处理toggle/show/hide情况
        // show/hide pass
        for (prop in props) {
            value = props[prop];
            // 如果设置属性动画形式为toggle/show/hide
        ...
        // 接着上面value为toggle/show/hide，orig需要在上面设置才不为空，对属性进行修正
        if (!jQuery.isEmptyObject(orig)) {
            ...
            // 第1步：对于hide->show的元素，先变为显示状态（否则从0到now的运动看不见）
            if (hidden) {
                jQuery(elem).show();
    ...
    // 单个属性对应的动画对象，功能包括初始化、计算当前值，计算下一动画属性值
    // 还包括获取/设置属性的兼容处理
    ...
    Tween.prototype = {
        ...
        // 计算该次运动后的值，并设置到style中
        run: function (percent) {
            var eased,
                hooks = Tween.propHooks[this.prop];

            if (this.options.duration) {
                // 可以在jQuery.easing中扩展运动函数，默认"swing"
    ...
    Tween.propHooks = {
        _default: {
            get: function (tween) {
                ...
                // dom节点上有该属性，且style上无该属性，获取属性值返回
                // 注意：此处获取的值是带单位的
    ...
    jQuery.fn.extend({
        ...
        animate: function (prop, speed, easing, callback) {
            var empty = jQuery.isEmptyObject(prop),
                ...
                doAnimation = function () {
                    ...
                    // 属性为空，或执行jQuery.fn.finish期间设置jQuery._data( this, "finish" )为true，所有动画创建后都必须立即结束到end
            ...
            return empty || optall.queue === false ?
                // 如果prop为空对象或者queue为false(即不使用动画队列)，遍历元素集并执行doAnimation回调
        ...
        stop: function (type, clearQueue, gotoEnd) {
            ...
            return this.each(function () {
                ...
                // 下面这一段应该是跟自定义有关，data缓存的对象没有直接设置过带stop方法的方法
                if (index) {
                ...
                for (index = timers.length; index--;) {
                    if (timers[index].elem === this && (type == null || timers[index].queue === type)) {
                        ...
                        // gotoEnd为true，stop内部会调用run(1)，并resolve触发promise，从而执行complete函数，从而dequeue下一个动画
                        timers[index].anim.stop(gotoEnd);
                        // 如果上面的gotoEnd为false，就不会自动dequeue了，需要下面手动dequeue到下一个
                        dequeue = false;
                        timers.splice(index, 1);
                    }
                }

                // 如果还有并且没被clearQueue的话，后续的动画需要继续进行
                // gotoEnd为false，或没有stop过动画（dequeue为ture），原来的动画仍会执行完从而触发下一队列，
                // 否则需要手动dequeue
        ...
        finish: function (type) {
            ...
            return this.each(function () {
                ...
                // 标记为finish阶段，此时所有的doAnimation执行时都会立即调用anim.stop(true)，直接到动画结束
    ...
    function genFx(type, includeWidth) {
        ...
        for (; i < 4; i += 2 - includeWidth) {
            // 不包含宽度，which就取“Top/Bottom”，否则“Left/Right”
            which = cssExpand[i];
    ...
    jQuery.speed = function (speed, easing, fn) {
        ...
        // jQuery.fx.off是否为真，是则将opt.duration设置为0，这将会停止所有动画
        opt.duration = jQuery.fx.off ? 0 :
            // 否则判断duration属性值是否为数字类型，是则使用
            typeof opt.duration === "number" ? opt.duration :
                // 否则判断duration属性值字符串是否是jQuery.fx.speeds(jQuery的预配置动画时长)属性，是则使用
                opt.duration in jQuery.fx.speeds ? jQuery.fx.speeds[opt.duration] :
                    // 否则就是用默认动画时长
                    jQuery.fx.speeds._default;

        // queue属性用来标识是动画队列，如果是false，不设置队列，立即开始动画
        // 如果opt.queue的值是true/undefined/null之一，将其值设置为"fx"字符串，标示动画队列
    ...
    // 保留jQuery.fx是为了兼容之前的版本
    // Tween.prototype.init是个构造函数，在fx上添加方法只是函数的静态方法，应该没有特殊用处，当作初始化
    ...
    jQuery.fx.tick = function () {
        ...
        for (; i < timers.length; i++) {
            timer = timers[i];
            // 如果当前tick返回的为假（经弱转换），移除该tick，然后继续遍历
    ...
    // 如果tick回调返回结果不是false，push到timers数组中，并开始动画
    ...
    // 扩展了伪类:animated，用于检测DOM元素是否正在执行动画
    if (jQuery.expr && jQuery.expr.filters) {
